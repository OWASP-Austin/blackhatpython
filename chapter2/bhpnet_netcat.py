'''
Note - the author named this file bhpnet.py,
I added netcat to the title for at a glance reading
To run : $ python bhpnet.py
...
# sys - commandline args
# socket - networking
# threading - allow for multiple listeners on same port
# getopt - c style parser for cmdline, parses flags
# subprocess - allows you to run subprocesses (e.g. shell on server)
...
def usage():
    ...
    print "-e --execute=file_to_run         -execute the given file upon receiving \
                                            a connection"
    print "-c --command                     -intitialize a command shell"
...
This is for TCP connections
    AF_INET is: address familiy
    SOCK_STREAM is: TCP (where is the UDP selection in this app?)
...
def server_loop():
    ...
    while True:
        ...
        # spint off a thread to handle our new client
